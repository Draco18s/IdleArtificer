...
namespace Assets.draco18s.util
{
	...
	public class BigInteger
	{
		...
		/// <summary>
		/// Creates a BigInteger from the parameters, e.g. "3E10" would be represented by the parameter values '3' and '10'.
		/// </summary>
		/// <param name="number">A long.</param>
		/// <param name="number">Power of 10</param>
		public BigInteger(long number, long power) {
			/*m_digits = new DigitsArray((8 / DigitsArray.DataSizeOf) + 1, 0);
			while(number != 0 && m_digits.DataUsed < m_digits.Count) {
				m_digits[m_digits.DataUsed] = (DType)(number & DigitsArray.AllBits);
				number >>= DigitsArray.DataSizeBits;
		...
		public static BigInteger operator *(double leftSide, BigInteger rightSide) {
		...
		public static int convertToInt(BigInteger v) {
			DType q = v.m_digits[0];
			if(q > int.MaxValue) {
				q = q & 0x7FFFFFFF;
			}
			return (int)q;
		...
		public static double logBigInteger(BigInteger val) {
			/*BigInteger valMutable = val;
			Int64 tens = 0;
			while(valMutable >= 10) {
				if(valMutable > 100000000) {
					tens += 8;
					valMutable /= 100000000;
				}
				else {
					tens++;
					valMutable /= 10;
