...
namespace Assets.draco18s.artificer.game {
	public class EnchantingManager {
		...
		//private static Dictionary<ItemStack, Transform> stackList = new Dictionary<ItemStack, Transform>();
...
#pragma warning restore 0219
		...
		public static void addInput1(Industry selectedIndustry) {
			if(selectedIndustry != null) {
				...
				selectedIndustry.enchantInvenListObj.transform.FindChild("Quantity").GetComponent<Text>().text = Main.AsCurrency(selectedIndustry.quantityStored) + " / " + Main.AsCurrency(inputStack1.stackSize);
		...
		private static void doOutput() {
			if(inputStack1 != null && inputStack2 != null) {
				...
				Debug.Log(inputStack1.item.name + "[" + inputStack1.enchants.Count + "]," + inputStack2.item.name + "+" + ench);
