...
namespace Assets.draco18s.artificer.game {
	public class TutorialManager {
		...
		private static void createCraftingTutorial() {
			...
			tutorialList.tutorialList.Add(new TutorialStep(new Vector3(-420, 225, 0), new Vector3(-260, 180, 0), "These are Industries. They are your primary way of making money. They produce goods of different kinds every 10 seconds.\nBuild the Wood industry to get started.", delegate { return true; }, delegate { return Main.instance.player.builtItems.Count > 0; }));
			tutorialList.tutorialList.Add(new TutorialStep(new Vector3(0, -70, 0), new Vector3(175, 170, 0), "You can click and drag to move industries around the crafting area and right-click it to advance its progress.\nYou can also left-click it to get more details, such as its inputs, consumers, current stock and more. Do that now.", delegate { return true; }, delegate { return GuiManager.instance.infoPanel.transform.localPosition.x > -400; }));
			tutorialList.tutorialList.Add(new TutorialStep(new Vector3(-100, 45, 0), new Vector3(0, 170, 0), "This shows the industry's level, allows you to upgrade (or downgrade) it, as well as to assign Apprentices (which you get access to much later).\nClick to continue.", delegate { return true; }, delegate { return Input.GetMouseButtonDown(0); }));
			...
			tutorialList.tutorialList.Add(new TutorialStep(new Vector3(0, -220, 0), new Vector3(-200, -310, 0), "You need to enable it for each industry first as well as set the desired upgrade level before any effect will take place.\nClick to continue.", delegate { return true; },
				delegate {
					CraftingManager.FacilitySelected(Main.instance.player.builtItems[0]);
					CraftingManager.ShowInfo();
					GuiManager.instance.infoPanel.transform.localPosition = new Vector3(0, 312, 0);
			...
			tutorialList.tutorialList.Add(new TutorialStep(new Vector3(110, -220, 0), new Vector3(-200, -310, 0), "This area deals with Vendors, which will automatically sell some resources after they are produced. You start with 3.\nAssign one now.", delegate { return true; },
				delegate {
					CraftingManager.FacilitySelected(Main.instance.player.builtItems[0]);
					CraftingManager.ShowInfo();
					GuiManager.instance.infoPanel.transform.localPosition = new Vector3(0, 312, 0);
			...
			tutorialList.tutorialList.Add(new TutorialStep(new Vector3(-420, 225, 0), new Vector3(-260, 180, 0), "Great! You're on your way to success. This is the end of this part of the tutorial.", delegate { return Main.instance.player.builtItems.Count > 1; }, delegate { return Input.GetMouseButtonDown(0); }));
		...
		private static void createGuildTutorial() {
			...
			tutorialList.tutorialList.Add(new TutorialStep(new Vector3(195, 345, 0), new Vector3(0, 200, 0), "Guild Hall unlocked!\nClick the Guild tab to learn more.", delegate { return StatisticsTracker.unlockedGuild.isAchieved(); }, delegate { return GuiManager.instance.guildArea.GetComponent<Canvas>().enabled; }));
			...
			tutorialList.tutorialList.Add(new TutorialStep(new Vector3(200, 60, 0), new Vector3(200, 50, 0), "This is the end of this part of the tutorial.", delegate { return true; }, delegate { return Input.GetMouseButtonDown(0); }));
			allTutorials.Add(tutorialList);
		}

		private static void createQuestTutorial() {
			TutorialList tutorialList = new TutorialList("quests");
			tutorialList.tutorialList.Add(new TutorialStep(new Vector3(-95, 345, 0), new Vector3(0, 200, 0), "Quests unlocked!\nClick the Quest tab to learn more.", delegate { return StatisticsTracker.unlockedQuesting.isAchieved(); }, delegate { return GuiManager.instance.questArea.GetComponent<Canvas>().enabled; }));
			...
			tutorialList.tutorialList.Add(new TutorialStep(new Vector3(5, 100, 0), new Vector3(5, 100, 0), "That's the end of this part of the tutorial. Keep it up!", delegate { return true; }, delegate { return Input.GetMouseButtonDown(0); }));
			allTutorials.Add(tutorialList);
		}

		private static void createEnchantingTutorial() {
			TutorialList tutorialList = new TutorialList("enchanting");
			tutorialList.tutorialList.Add(new TutorialStep(new Vector3(-95, 345, 0), new Vector3(0, 200, 0), "Enchanting unlocked!\nClick the Quest tab to learn more.", delegate { return StatisticsTracker.unlockedEnchanting.isAchieved(); }, delegate { return GuiManager.instance.questArea.GetComponent<Canvas>().enabled; }));
		...
		private static void createResearchTutorial() {
			...
			tutorialList.tutorialList.Add(new TutorialStep(new Vector3(-95, 345, 0), new Vector3(0, 200, 0), "Research unlocked!\nClick the Quest tab to learn more.", delegate { return StatisticsTracker.unlockedResearch.isAchieved(); }, delegate { return GuiManager.instance.questArea.GetComponent<Canvas>().enabled; }));
			...
			tutorialList.tutorialList.Add(new TutorialStep(new Vector3(190, 85, 0), new Vector3(240, 130, 0), "That's everything!\nClose the relic info window to complete this part of the tutorial", delegate { return true; }, delegate { return !GuiManager.instance.researchArea.transform.Find("RelicInfoOpen").gameObject.activeSelf; }));
